...
func (r *PersistentVolumeProvisionerReconciler) createPersistentVolume(ctx context.Context, pod corev1.Pod, pvc corev1.PersistentVolumeClaim) error {
        ...
        // Get target node name - either from pod.Spec.NodeName or from nodeAffinity
        targetNodeName := pod.Spec.NodeName

        // If pod.Spec.NodeName is empty, try to extract it from the pod's nodeAffinity
        if targetNodeName == "" && pod.Spec.Affinity != nil && pod.Spec.Affinity.NodeAffinity != nil {
                        ...
                        for _, term := range nodeAffinity.RequiredDuringSchedulingIgnoredDuringExecution.NodeSelectorTerms {
                                for _, expr := range term.MatchExpressions {
                                        if expr.Key == "kubernetes.io/hostname" && expr.Operator == corev1.NodeSelectorOpIn && len(expr.Values) > 0 {
                                                targetNodeName = expr.Values[0]
                                                logger.Info("Found target node from pod nodeAffinity", "node", targetNodeName)
                                                break
                                        }
                                }
                                if targetNodeName != "" {
        ...
        // Check if every element of preferred path is available
        if r.PvPath != "" && pod.Namespace != "" && releaseName != "" && targetNodeName != "" {
                ...
                // Fallback - dynamic path building with available elements
                if r.PvPath != "" {
                        localPath = r.PvPath
        ...
        // Create NodeAffinity based on either pod.Spec.NodeName or nodeAffinity from pod spec
        if targetNodeName != "" {
                logger.Info("Adding NodeAffinity to PV", "nodeName", targetNodeName)
                ...
                logger.Info("Pod doesn't have assigned node or nodeAffinity, creating PV without NodeAffinity", "pod", pod.Name)
                // nodeAffinity remains nil
        }
